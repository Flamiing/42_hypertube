...
export default class TorrentClient {
    ...
    async #parseTorrent() {
        ...
        try {
            ...
            const torrent = bencode.decode(torrentFile, { decodeStrings: false });
            return torrent;
        } catch (error) {
            console.error('Error reading or parsing the torrent file:', error.message);
            return null;
        }
    }
        ...
        for (const announce of allTrackers) {
            try {
                ...
                if (!announceURL.startsWith('http')) {
                    console.warn('[TRACKER] Skipping unsupported URL (likely UDP):', announceURL);
                    continue;
                }
                ...
                console.log('[TRACKER] Requesting peers from:', `${parsedURL.hostname}${fullPath}`);
                ...
                const req = requestLib.request(options, (res) => {
                    ...
                    res.on('end', () => {
                        ...
                        try {
                            ...
                            console.error('[TRACKER] Failed to decode response from', announceURL, ':', e.message);
                        }
                    });
                });
    
                req.on('error', (err) => {
                    console.error('[TRACKER] Request error from', announceURL, ':', err.message);
                });
    
                req.end();
            } catch (e) {
                console.error('[TRACKER] Error handling tracker URL:', e.message);
            }
        }
    }
